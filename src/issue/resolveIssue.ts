...
import { QuickPickItem, Uri, window } from "coc.nvim";
...
export async function resolveIssueMultiStepInput(
    ...
    async function resolveIssue(input: MultiStepInput, state: Partial<State>) {
        const response = isDependencyRisk
            ? await IssueService.instance.checkDependencyRiskStatusChangePermitted(issueKey)
            : await IssueService.instance.checkIssueStatusChangePermitted(workspaceFolderUri, issueKey);
        if (!response || response == undefined) {
            window.showWarningMessage(
                `Unable to locate issue with key [${issueKey}], issue key was invalid for ${Uri.parse(workspaceFolderUri).fsPath}`
            );
            return;
        }
        if (!response.permitted) {
            window.showWarningMessage(
                `Resolving issue not permitted due to [${response.notPermittedReason}] for ${Uri.parse(workspaceFolderUri).fsPath}`
            );
            return;
        }
        const allowedStatuses: IndexQP[] = response.allowedStatuses.map((label, index) => ({
            label: isDependencyRisk ? DependencyRiskTransition[label] : label,
            index
        }));
        ...
        if (pickedIssueStatus) {
            ...
            showChangeStatusConfirmationDialog(changeStatusType).then(async (answer) => {
                if (answer === "Yes") {
                    if (isDependencyRisk) {
                        await IssueService.instance.changeDependencyRiskStatus(
                            workspaceFolderUri,
                            issueKey,
                            response.allowedStatuses[pickedIssueStatus.index],
                            comment
                        );
                    } else {
                        await IssueService.instance.changeIssueStatus(
                            workspaceFolderUri,
                            issueKey,
                            pickedIssueStatus.label,
                            fileUri,
                            comment,
                            isTaintIssue
                        );
    ...
    async function inputComment(input: MultiStepInput, state: Partial<State>, isDependencyRisk: boolean) {
        const prompt = isDependencyRisk
            ? "Provide a reason for changing the dependency risk status (required for 'Accept' and 'Safe' transitions)"
    ...
    async function pickIssueStatus(input: MultiStepInput, title: string, resolveTransitions: IndexQP[]): Promise<IndexQP> {
