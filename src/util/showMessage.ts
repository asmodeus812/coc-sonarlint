...
export async function tooManyFilesConfirmation(filesCount: number): Promise<string | undefined> {
...
export function showChangeStatusConfirmationDialog(changeStatusType: ChangeStatusType) {
    ...
    const noteAboutQualityGate =
        changeStatusType === "issue" ? ` A resolved ${changeStatusType} will be ignored when assessing the Quality Gate.` : "";
    return window.showInformationMessage(`Do you want to do this?. ${message + noteAboutQualityGate}`, "Yes");
}
...
export async function showSslCertificateConfirmationDialog(cert: ExtendedClient.SslCertificateConfirmationParams) {
    ...
    const dialogResponse = await coc.window.showErrorMessage(
        `
...
export function showNoFileWithUriError(uri: coc.Uri) {
    coc.window
        .showErrorMessage(
        ...
        .then((action) => {
            if (action === "Show Documentation") {
                coc.commands.executeCommand(
                    ...
                    coc.Uri.parse("https://docs.sonarsource.com/sonarqube-for-vs-code/troubleshooting/#no-matching-issue-found")
                );
            }
        });
}
