...
function platformToJreOs(platform: string): string {
    return {
        win32: "windows",
        linux: "linux",
        darwin: "mac"
...
function archToJreArch(arch: string): string {
    return {
        x86_64: "x64",
        x86: "x32"
    }[arch] as string;
...
export class PlatformInformation {
    ...
    private static async GetUnixArchitecture(): Promise<string> {
        return util
            .execChildProcess("uname -m", util.extensionPath)
            .then((architecture) => {
                if (architecture) {
