...
export async function maybeShowWiderLanguageSupportNotification(context: coc.ExtensionContext, languages: string[]) {
    ...
    const notShownNotificationInCurrentPromoPeriod =
        lastShowedNotificationAt === 0 || (Date.now() - lastShowedNotificationAt) / MILLISECONDS_IN_A_DAY >= PROMOTIONAL_PERIOD_DAYS;
    ...
    if (areNotificationsEnabled && notShownNotificationInCurrentPromoPeriod) {
        ...
        const message =
            languages.length > 1
                ? `Enable ${languages[0]} or ${languages[1]} analysis by setting up SonarLint Connected Mode.`
                : `Enable ${languages[0]} analysis by setting up SonarLint Connected Mode.`;
        const selection = await coc.window.showInformationMessage(message, learnMoreAction, dontShowAgainAction);
