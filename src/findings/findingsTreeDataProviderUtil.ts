...
export const impactSeverityToIcon = (impactSeverity: ExtendedServer.ImpactSeverity): { light: coc.Uri; dark: coc.Uri } => {
...
export function isFileOpen(fileUri: string): boolean {
    return coc.workspace.textDocuments.some((doc) => doc.uri.toString() === fileUri);
}

export function isCurrentFile(fileUri: string): boolean {
    const activeEditor = coc.window.activeTextEditor;
    return activeEditor?.document.uri === fileUri;
}
...
export async function selectAndApplyCodeAction(codeActions: coc.CodeAction[]) {
    const selection: IndexQP | undefined = await coc.window.showQuickPick(
    ...
    if (selection) {
        ...
        if (selectedAction.edit) {
            try {
                await coc.workspace.applyEdit(selectedAction.edit);
                await coc.commands.executeCommand(selectedAction.command?.command as string, ...(selectedAction?.command?.arguments || []));
            } catch (error) {
                coc.window.showErrorMessage(`Error applying quick fix: ${(error as Error).message}`);
            }
        } else if (selectedAction.command) {
            await coc.commands.executeCommand(selectedAction.command.command, ...(selectedAction.command.arguments || []));
        } else {
            coc.window.showWarningMessage("Selected Code Action has no edit or command to execute.");
        }
    }
}
