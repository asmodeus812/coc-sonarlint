...
export function highlightAllCodeSnippetsInDesc(htmlDescription: string, ruleLanguageKey: string, diffed: boolean) {
    ...
    try {
        ...
        preTagsNoDiff.forEach((pre: { textContent: string; innerHTML: string }) => {
            const highlightedCode = hljs.highlight(pre.textContent, {
                language: languageKey,
                ignoreIllegals: true
            });
            pre.innerHTML = `<code class="${language}">${highlightedCode.value}</code>`;
            return pre;
        });
        ...
        if (diffed) {
            ...
            preTagsWithDiff.forEach((pre: any) => {
                ...
                codeDiffContainer.querySelectorAll("div").forEach((div, index) => {
                    ...
                    newDiv.innerHTML = `<code class="${language}">${highlightedCode.value}</code>`;
                    div.remove();
