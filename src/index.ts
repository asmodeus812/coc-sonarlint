...
function registerCommands(context: coc.ExtensionContext) {
    ...
    context.subscriptions.push(
        coc.commands.registerCommand(Commands.TOGGLE_RULE, toggleRule(undefined)),
    );
    ...
    context.subscriptions.push(
        coc.commands.registerCommand(
            ...
            async (ruleKey: string) => {
                await coc.commands.executeCommand(Commands.SHOW_ALL_RULES);
                await allRulesView?.reveal(
                    new RuleNode({key: ruleKey.toUpperCase()} as protocol.Rule),
                    {
                        focus: true,
                        expand: true,
                    },
