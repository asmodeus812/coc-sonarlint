    ...
    if (extension?.isActive) {
        if (!classpathChangeListener) {
            ...
            if (extensionApi && isJavaApiRecentEnough(extensionApi.apiVersion)) {
                const onDidClasspathUpdate: coc.Event<coc.Uri> = extensionApi.onDidClasspathUpdate;
                classpathChangeListener = onDidClasspathUpdate(function (uri) {
                    languageClient.didClasspathUpdate(uri);
                });
                logToSonarLintOutput(`Registered a new classpath listenere for the java extension`);
            }
        }
    ...
    if (extension?.isActive) {
        if (!serverModeListener) {
            ...
            if (extensionApi && isJavaApiRecentEnough(extensionApi.apiVersion) && extensionApi.onDidServerModeChange) {
                const onDidServerModeChange: coc.Event<ServerMode> = extensionApi.onDidServerModeChange;
...
function isJavaApiRecentEnough(apiVersion: string): boolean {
    ...
    if (!javaApiTooLowAlreadyLogged) {
        logToSonarLintOutput(`Sonarlint requires extension 0.56 or greater to enable analysis of Java files`);
...
export async function getJavaConfig(
    ...
    fileUri: string
): Promise<ExtendedClient.GetJavaConfigResponse | null | undefined> {
    const extension = getJavaExtension();
    try {
...
function javaConfigDisabledInLightWeightMode() {
    if (!javaServerInLightWeightModeAlreadyLogged) {
        logToSonarLintOutput(`Java analysis is disabled in LightWeight mode. Please check java.server.launchMode in user settings`);
