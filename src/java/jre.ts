...
export function buildUrl(options: Options) {
    ...
    const requestUrl = [ADOPT_OPEN_JDK_API_ROOT, actualOptions.request, actualOptions.release, `openjdk${actualOptions.version}`].join("/");
    const requestParams = [
...
export function unzip(downloadResponse: DownloadResponse) {
    ...
    return new Promise((resolve, reject) => {
        ...
        extract.on("end", () => {
            ...
            const actualJavaHome = downloadResponse.options.os === "mac" ? path.join(extractedDir, "Contents", "Home") : extractedDir;
